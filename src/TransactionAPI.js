export const transactions = {
  "NWYyYmYwZGVjNzMyNzk1MTgyZTc3OTFkNTkwMjcyNjk=": {
    ...
    card: {
      user: {
        id: "VXNlcjpjdXVzZXJfYzFoaWttazBzMDAwMTc3M3kxczBpMXhr",
        firstName: "Jonathan",
        lastName: "Lima",
      },
    },
...
export const localStorageKey = "transactionState";
export const state = (() => {
  let internalState = {
  ...
  const load = () => {
    ...
    if (storedState) {
      internalState = JSON.parse(storedState);
    }
  };
  const save = () => {
    localStorage.setItem(localStorageKey, JSON.stringify(internalState));
  };
  load();
  return {
    categories: () => internalState.categories,
    transactions: () => internalState.transactions,
    load,
    save,
    updateTransaction(transactionId, key, value) {
      internalState = {
        ...internalState,
        transactions: {
          ...internalState.transactions,
          [transactionId]: {
            ...internalState.transactions[transactionId],
            [key]: value,
          },
        },
      };
...
export async function getTransactions() {
  return new Promise((resolve) => {
    setTimeout(() => resolve(state.transactions()), 200);
  });
}
export async function getCategories() {
  return new Promise((resolve) => {
    setTimeout(() => resolve(state.categories()), 200);
  });
}
export async function setTransactionCategory(transactionId, categoryId) {
  return new Promise((resolve) => {
    setTimeout(() => {
      state.updateTransaction(transactionId, "categoryId", categoryId);
      state.save();
      return resolve(state.transactions()[transactionId]);
    }, 200);
  });
}
export async function setTransactionMemo(transactionId, memo) {
  return new Promise((resolve) => {
    setTimeout(() => {
      state.updateTransaction(transactionId, "memo", memo);
      state.save();
      return resolve(state.transactions()[transactionId]);
    }, 200);
  });
